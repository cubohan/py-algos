#!/usr/bin/python
"""
cmd_interpret.py by rbcorx

NOTE: python 3 incompatible. Run with python 2.x `python cmd_interpret.py`

'Plug and Play' Command line arguments parser and compiler (akin to custom grid search)
    
Best used to test various functions with different parameters 
    (EG: For parametric tuning) passed from the commandline in real time. 
    Supports automatic range based parameter testing.
...
try:
    # windows sound package
    import winsound
except ImportError:
    # alternative for linux system
    import os
    def playsound(frequency,duration):
        # NOTE: apt-get install beep for linux
        ...
        if self._generators: # Partial Permutation yield is also available
            try:
                if self.permute_ranges:
                    ...
                    opts[gen] = next(self._generators[gen])
